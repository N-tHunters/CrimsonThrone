...
void DungeonA1Generator::GenerateDungeon(int tx, int ty, int width, int height) {
  ...
  do {
    ...
    if (dx != 0) {
      if (dx == -1) {
        walls[RIGHT_WALL(ty, tx)] = false;
      } else {
        walls[LEFT_WALL(ty, tx)] = false;
      }
    } else {
      if (dy == -1) {
        walls[UP_WALL(ty, tx)] = false;
      } else {
        walls[DOWN_WALL(ty, tx)] = false;
      }
    }
